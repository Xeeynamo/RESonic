...
namespace XeLib
{
    public class GFX
    {
        ...
        struct GFXHeader
        {
            ...
            public void Read(FileStreamEx f)
            {
                headersize = f.Read8();
                dstindex = f.Read8();
                width = f.Read8();
                height = f.Read8();
                resolution = f.Read8();
                texturecount = f.Read8();
                mode = f.Read8();
                flags = f.Read8();
            }
            public void Write(FileStreamEx f)
            {
        ...
        public bool Open(string filename, ref Bitmap bmp)
        {
            try
            {
                FileStreamEx f = new FileStreamEx(filename, FileMode.Open, FileAccess.Read);
                head.Read(f);
        ...
        public void SaveRawData(ref Bitmap bmp, FileStreamEx f)
        {
        ...
        {
            FileStreamEx f = new FileStreamEx(filename, FileMode.Create, FileAccess.Write);
            head.Write(f);
            SaveRawData(ref bmp, f);
        ...
        public void Create(short width, short height, PixelFormat pixelformat, byte[] data, ref Bitmap bmp)
        {
            ...
            Rectangle lockRect = new Rectangle(0, 0, bmp.Width, bmp.Height);
            BitmapData bmpdata = bmp.LockBits(lockRect, ImageLockMode.WriteOnly, bmp.PixelFormat);
            int size = bmpdata.Width * bmpdata.Height * BitsPerPixel / 8;
            System.Runtime.InteropServices.Marshal.Copy(data, 0, bmpdata.Scan0, size);
            bmp.UnlockBits(bmpdata);
        }
